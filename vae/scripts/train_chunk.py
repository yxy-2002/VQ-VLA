...
def get_args():
    p = argparse.ArgumentParser(description="Train chunked hand-action VAE")
...
def main():
    ...
    print(
        ...
        f"latent={args.latent_dim}, beta={args.beta}, noise_std={args.noise_std}"
    )
    ...
    for step in range(args.total_steps):
        ...
        window, target = window.to(device), target.to(device)
        _, recon_loss, kl_loss, total_loss, _, _ = model(window, target)
