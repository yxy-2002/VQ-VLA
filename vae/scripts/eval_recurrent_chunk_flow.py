"""
Evaluate the recurrent latent flow hand prior.
"""
...
def get_args():
    parser = argparse.ArgumentParser(description="Evaluate recurrent chunk flow prior")
...
@torch.no_grad()
def rollout_free_batch(model, num_rollouts, max_steps, integration_steps):
    ...
    t = 1
    while t < max_steps:
        pred, z, hidden, current_state = model.sample_chunk(
            hidden,
            current_state,
...
def main():
    ...
    model = HandActionRecurrentChunkFlow(**infer_model_args(ckpt)).to(device)
